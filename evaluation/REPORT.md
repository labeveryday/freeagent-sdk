# FreeAgent SDK — Evaluation Report

Comprehensive evaluation comparing raw Ollama API vs Strands Agents SDK vs FreeAgent SDK.
Baseline results establish the performance floor; FreeAgent results prove the thesis.
...
| Multi-Turn (no state) | 93% (qwen3:8b) | 80% (qwen3:4b) | 78% (all models) |
| Multi-Turn (conversation) | 93% (qwen3:8b) | 80% (qwen3:4b) | **87% (qwen3:8b, qwen3:4b)** |
...
4. **Conversation manager closes the multi-turn gap.** With `SlidingWindow` (default), FreeAgent jumps from 78% to 87% on multi-turn, beating Strands (73-80%) and approaching raw Ollama (87-93%). See `evaluation/CONVERSATION_REPORT.md` for full comparison.

5. **gemma4:e2b (2B, ReactEngine) achieves 80% on multi-turn** — matching llama3.1 (8B, native tools) at 1/4 the size. ReactEngine successfully parses tool calls from text output with no parse errors.
